...
# def resize_for_display(img: np.ndarray, max_width: int = 1100, max_height: int = 700) -> np.ndarray:
#     h, w = img.shape[:2]
...
#         c1, c2 = st.columns(2)
#         with c1:
#             st.image(bgr_to_rgb(resize_for_display(image_bgr)), caption="Original", width="stretch")
#         with c2:
#             st.image(bgr_to_rgb(resize_for_display(annotated)), caption="Detection Output", width="stretch")
...
#                 preview.image(bgr_to_rgb(resize_for_display(annotated)), caption="Processing preview", width="stretch")
#             progress.progress(min(current / frame_count, 1.0))
...
#                 alarm_enabled=False
#             )
#             st.image(bgr_to_rgb(resize_for_display(annotated)), caption="Browser webcam detection", width="stretch")
#             if perf_rows:
#                 st.dataframe(pd.DataFrame(perf_rows), width="stretch")
#             perf_placeholder.json(st.session_state.last_perf)
...
#                     )
#                     frame_holder.image(bgr_to_rgb(resize_for_display(annotated)), channels="RGB", width="stretch")
#                     if perf_rows:
